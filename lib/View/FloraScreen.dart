...
class FloraScreen extends StatefulWidget {
  ...
  @override
  _FloraScreenState createState() => _FloraScreenState();
}
...
class _FloraScreenState extends State<FloraScreen> {
  ...
  Widget vistaAdmin() {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance.collection('flora').snapshots(),
  ...
  void _eliminarContenido(String docId) async {
    ...
    if (confirmacion == true) {
      await FirebaseFirestore.instance.collection('flora').doc(docId).delete();
    }
  }
  ...
  Future<void> _agregarContenido(XFile imagen, String descripcion) async {
    try {
      if (imagen != null) {
        ...
        await FirebaseFirestore.instance.collection('flora').add({
          'imageUrl': imageUrl,
